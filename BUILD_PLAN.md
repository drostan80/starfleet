...
- [ ] **B.5.4 — Prove B.5.1–B.5.3 stable in real use**, then proceed to
  ...
  **Flipped `[x]` on 2026-08-15 evidence (36 webhook hits, 3 clean
  activity-poll ticks — see git history for that entry's original
  text), then flipped back to `[ ]` the same night** — the user caught,
  live, minutes later: Ascendance of a Bookworm's currently-airing part
  had been silently invisible in Data because `_fetch_sonarr`
  (`metadata.py`) was writing Sonarr's raw episode numbering straight
  into whichever one show held the tvdb link, no check against LCARS's
  own season model at all — a direct violation of `SCOPE.md` §3's own
  founding principle ("the internal database is the source of truth...
  mapped in, never authoritative over internal behavior"), not a minor
  edge case. B.5.1/B.5.3's own webhook/activity-poll evidence still
  stands on its own terms, but B.5.4 asks whether **Phase B as a
  whole** does what it was built to do, and this proved live, in
  production, that a core piece of it didn't. Fixed the same night
  (`_fetch_sonarr_multi_show`, routes each episode to whichever sibling
  show its own episode history actually continues, by air-date
  continuity — never guesses, opens `pending_review` instead when a gap
  can't be resolved with confidence) — see the "Bugs" section of
  `todo.md` for the full incident writeup. Left unchecked again
  deliberately: one bug found and fixed the same night it was caught is
  not the same evidence bar B.5.1/B.5.3 met over real elapsed time, and
  finding this one live raises the honest question of what else in
  Phase B hasn't been exercised against a real multi-part/edge-case
  library yet. Re-flip only on new evidence, not on "it's fixed now."
