...
def upgrade() -> None:
    ...
    # 2. Mark seasons as 'watching' where they have at least one
    #    watched/skipped episode but aren't fully completed.
    op.execute(
    ...
    # 3c. Shows where highest season has a non-planned status that
    #     differs from current show.status (and show isn't completed)
    op.execute(
